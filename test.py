...
class Robot:
    ...
    def draw(self):
        u = u0 + k * self.x
        v = v0 - k * self.y
        pygame.draw.circle(screen, white, (int(u), int(v)),
                           int(k * ROBOTRADIUS), 3)
        ...
        vlx = v0 - k * wly
        pygame.draw.circle(
            screen, blue, (int(ulx), int(vlx)), int(k * WHEELBLOB))
        # right wheel centre
        ...
        vrx = v0 - k * wry
        pygame.draw.circle(
            screen, blue, (int(urx), int(vrx)), int(k * WHEELBLOB))
        ...
        for path in self.pathstodraw:
            ...
            if path[0] == 0:    # Straight line
                ...
                lineend = (u0 + k * (self.x + straightpath * math.cos(self.theta)),
                           v0 - k * (self.y + straightpath * math.sin(self.theta)))
...
for i in range(20):
    (bx, by, vx, vy) = (random.uniform(PLAYFIELDCORNERS[0], PLAYFIELDCORNERS[2]), random.uniform(
        PLAYFIELDCORNERS[1], PLAYFIELDCORNERS[3]), random.gauss(0.0, BARRIERVELOCITYRANGE), random.gauss(0.0, BARRIERVELOCITYRANGE))
    barrier = [bx, by, vx, vy]
...
def predictPosition(vL, vR, x, y, theta, deltat):
    ...
    # Pure rotation motion
        ...
        # Turn this into Rect
        Rabs = abs(R)
        ((tlx, tly), (Rx, Ry)) = ((int(u0 + k * (cx - Rabs)), int(v0 -
                                                                  k * (cy + Rabs))), (int(k * (2 * Rabs)), int(k * (2 * Rabs))))
        if (R > 0):
            start_angle = theta - math.pi/2.0
        else:
            start_angle = theta + math.pi/2.0
        stop_angle = start_angle + deltatheta
        path = (2, ((tlx, tly), (Rx, Ry)), start_angle,
                stop_angle)  # 2 indicates general motion
...
def chooseAction(vL, vR, x, y, theta):
    ...
    vLpossiblearray = (vL - MAXACCELERATION * dt,
                       vL, vL + MAXACCELERATION * dt)
    vRpossiblearray = (vR - MAXACCELERATION * dt,
                       vR, vR + MAXACCELERATION * dt)
    ...
    for vLpossible in vLpossiblearray:
        for vRpossible in vRpossiblearray:
            ...
            if (vLpossible <= MAXVELOCITY and vRpossible <= MAXVELOCITY and vLpossible >= -MAXVELOCITY and vRpossible >= -MAXVELOCITY):
                ...
                (xpredict, ypredict, thetapredict, path) = predictPosition(
                ...
                distanceToObstacle = calculateClosestObstacleDistance(
                    xpredict, ypredict)
                # Calculate how much close we've moved to target location
                previousTargetDistance = math.sqrt(
                    (x - barriers[targetindex][0])**2 + (y - barriers[targetindex][1])**2)
                newTargetDistance = math.sqrt(
                    (xpredict - barriers[targetindex][0])**2 + (ypredict - barriers[targetindex][1])**2)
                ...
                if (distanceToObstacle < SAFEDIST):
                    obstacleCost = OBSTACLEWEIGHT * \
                        (SAFEDIST - distanceToObstacle)
                else:
                    obstacleCost = 0.0
...
def drawBarriers(barriers):
    for (i, barrier) in enumerate(barriers):
        if (i == targetindex):
            ...
            bcol = lightblue
        pygame.draw.circle(screen, bcol, (int(
            u0 + k * barrier[0]), int(v0 - k * barrier[1])), int(k * BARRIERRADIUS), 0)
...
while (1):
    ...
    for robot in robotslistrandomised:
        ...
        (robot.vL, robot.vR, robot.pathstodraw) = chooseAction(
            robot.vL, robot.vR, robot.x, robot.y, robot.theta)

        (predx, predy, predtheta, tmppath) = predictPosition(
            robot.vL, robot.vR, robot.x, robot.y, robot.theta, TAU)
    ...
    for robot in robots:
        (robot.x, robot.y, robot.theta, tmppath) = predictPosition(
            robot.vL, robot.vR, robot.x, robot.y, robot.theta, dt)
    ...
    for robot in robots:
        disttotarget = math.sqrt(
            (robot.x - barriers[targetindex][0])**2 + (robot.y - barriers[targetindex][1])**2)
    ...
    if (mindist < (BARRIERRADIUS + ROBOTRADIUS)):
        # Add new barriers
        for i in range(2):
            (bx, by) = (random.uniform(PLAYFIELDCORNERS[0], PLAYFIELDCORNERS[2]), random.uniform(
                PLAYFIELDCORNERS[1], PLAYFIELDCORNERS[3]))
            (bx, by, vx, vy) = (random.uniform(PLAYFIELDCORNERS[0], PLAYFIELDCORNERS[2]), random.uniform(
                PLAYFIELDCORNERS[1], PLAYFIELDCORNERS[3]), random.uniform(-BARRIERVELOCITYRANGE, BARRIERVELOCITYRANGE), random.uniform(-BARRIERVELOCITYRANGE, BARRIERVELOCITYRANGE))
            barrier = [bx, by, vx, vy]
            barriers.append(barrier)
