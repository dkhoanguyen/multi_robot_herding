...
class DataAnalyser(object):
    ...
    def extract_herd_mean_to_target(self):
        ...
        target = np.array([1000, 350])
        target_grp = np.linalg.norm(
            np.zeros((len(self._herd_states), 2)), axis=1)
        diff = np.linalg.norm(herd_centroids - target,axis=1)
        fig = plt.figure()
        ax = fig.add_subplot(1, 1, 1)
        ax.plot(target_grp, linewidth=2, snap=True)
        ax.plot(diff, linewidth=2, snap=True)
        plt.show()
    ...
    def extract_and_plot_distribution_range(self):
        ...
        for indx, shepherd_state in enumerate(self._shepherd_states):
            ...
            d_to_herd_centroid = np.linalg.norm(
                shepherd_state_states[:, :2] - herd_centroid, axis=1)
    ...
    def extract_and_plot_distribution_eveness(self):
        herd_distribution_eveness = np.zeros(len(self._shepherd_states))
        herd_distribution_mean = np.zeros(len(self._shepherd_states))
        for indx, shepherd_state in enumerate(self._shepherd_states):
            states = shepherd_state["shepherd"]
        ...
        shepherd_distribution_eveness = np.zeros(len(self._shepherd_states))
        shepherd_distribution_mean = np.zeros(len(self._shepherd_states))
        for indx, shepherd_state in enumerate(self._shepherd_states):
            states = shepherd_state["shepherd"]
        ...
        fig = plt.figure()
        ax = fig.add_subplot(1, 1, 1)
        ax.plot(herd_distribution_eveness, linewidth=2)
        ax.plot(shepherd_distribution_eveness, linewidth=2)
        # ax.plot(herd_distribution_mean, linewidth=2)
        # ax.plot(shepherd_distribution_mean, linewidth=2)
        # ax.axhline(y=130, color="blue", linestyle="--", linewidth=2)
        # ax.axhline(y=270, color="orange", linestyle="--", linewidth=2)

        ax.set_title("Distribution Eveness", fontdict={
                     'fontsize': 14})
        ax.set_xlabel("Timestep")

        ax.set_ylim([0, herd_distribution_eveness.max()+1000])
        ax.set_xlim([0, len(self._herd_states)])
        ...
        # ax.annotate("Surrounding\nphase",[350,13000])
        # x = np.arange(0,len(self._herd_states),1)
        # ax.fill_between(x=x,y1=herd_distribution_eveness.max()+10000,where=x<=275,alpha=0.4)
        # ax.annotate("",
...
if __name__ == '__main__':
    ...
    analyser.extract_herd_mean_to_target()
